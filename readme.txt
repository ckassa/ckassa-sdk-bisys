...
Также указывается пароль для генерации хеша и список разрешенных адресов для запросов
Оператор должен добавить свой код проведения платежа в функцию do_payment в файле payment.php
Если пополнение платежа происходит внешним скриптом, то строчку 292 в файле payment.php (response(90, "Временная техническая ошибка");) можно расскоментировать.
Оператор может изменить структуру таблиц и код обработки запросов при необходимости.	

Готовый скипт можно протестировать локально из браузера с помощью файла test.html. Вставляйте в поле XML-запросы и отправляйте.
Пример запроса есть в протоколе. Для этого тестирования проверку подписи можно отключить. 
